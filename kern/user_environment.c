...
void test_clear_page_dir_entry(struct Env* ptr_env, uint32 virtual_address)
{
...
void __env_free_with_buffering(struct Env *e)
{
	    ...
	    uint32 worksetSize = env_page_ws_get_size(e);

	    if (worksetSize == e->page_WS_max_size) {
	        for (int i = 0; i < e->page_WS_max_size; i++) {
	            uint32* page_ptr = NULL;
	            uint32 VA = env_page_ws_get_virtual_address(e, e->page_last_WS_index);
	            uint32 perms = pt_get_page_permissions(e, VA);

	            if ((perms & PERM_BUFFERED) != 0) {
	                struct Frame_Info *fi = get_frame_info(e->env_page_directory, (void *)VA, &page_ptr);
	    ...
	    for (uint32 i = 0; i < 1024*PAGE_SIZE; i += PAGE_SIZE) {
	        uint32 *page_table_ptr = NULL;
	        get_page_table(e->env_page_directory, (void *)i, &page_table_ptr);
