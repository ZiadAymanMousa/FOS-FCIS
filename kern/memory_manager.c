...
//	size = ROUNDUP(size, PAGE_SIZE);
	int numofmahgozeen = size / PAGE_SIZE;
	for (int i = 0; i < numofmahgozeen; i++) {
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address,
		uint32 size) {
//sodo : hgeb el info w akaren el permissions b3d kda ased el bufferd w el env
	    uint32 * page_table_ptr = NULL;
	    uint32 VA = virtual_address;
	    uint32 sz = size + virtual_address;
//hna b4aw 3al page table entries a bgeb el frame info 3shan el permissions
	    get_page_table(e->env_page_directory, (void*) VA, &page_table_ptr);
// documentation
	    uint32 pgsize = PAGE_SIZE;
	    struct Frame_Info * frame_info_ptr = 0;
	    for (uint32 i = VA; i < sz; i += pgsize) {
	        uint32 permissions = pt_get_page_permissions(e, i);
	        if (permissions & PERM_BUFFERED) {
	            if (permissions & PERM_MODIFIED) {
	                frame_info_ptr = get_frame_info(e->env_page_directory, (void*) i, &page_table_ptr);
	                bufferlist_remove_page(&modified_frame_list, frame_info_ptr);
	                frame_info_ptr->isBuffered = 0;
	                frame_info_ptr->environment = 0;
	            ...
	            pt_clear_page_table_entry(e, i); //clearing entry in case it was buffered
	        }
	    }

	    for (uint32 j = VA; j < sz; j += pgsize) {
	        int KDA = 0;
	        while (KDA < e->page_WS_max_size) {
	            if (e->ptr_pageWorkingSet[KDA].virtual_address == j) {
	                env_page_ws_clear_entry(e, KDA);
	                unmap_frame(e->env_page_directory, (void*) j);
	                pt_clear_page_table_entry(e, j);
	            }
	            KDA++;
	        }
	    }

	    for (int j = VA; j < sz; j += pgsize) {
	        uint32* page_ptr = NULL;
	        get_page_table(e->env_page_directory, (void*) j, &page_ptr);
	        int TT = 0;
	        if (page_ptr != 0) {
	    ...
	    for (uint32 p = VA; p < sz; p += pgsize) {
	        pf_remove_env_page(e, p);
	    }
